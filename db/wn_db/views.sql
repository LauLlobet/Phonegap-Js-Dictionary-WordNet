...
CREATE  VIEW wordsXsenses AS SELECT * FROM words INNER JOIN senses USING (wordid);
CREATE  VIEW wordsXsensesXsynsets AS SELECT * FROM words INNER JOIN senses USING (wordid) INNER JOIN synsets USING (synsetid);
CREATE  VIEW adjectiveswithpositions AS SELECT * FROM senses INNER JOIN adjpositions USING (wordid,synsetid) LEFT JOIN words USING (wordid) LEFT JOIN synsets USING (synsetid);
...
CREATE  VIEW morphology AS SELECT * FROM words INNER JOIN morphmaps USING(wordid) INNER JOIN morphs USING(morphid);
...
CREATE  VIEW synsetsXsemlinksXsynsets AS SELECT linkid,s.synsetid AS ssynsetid,s.definition AS sdefinition,d.synsetid AS dsynsetid,d.definition AS ddefinition FROM synsets AS s INNER JOIN semlinks AS l ON s.synsetid = l.synset1id INNER JOIN synsets AS d ON l.synset2id = d.synsetid;
CREATE  VIEW verbswithframes AS SELECT * FROM senses INNER JOIN vframemaps USING (wordid,synsetid) INNER JOIN vframes USING (frameid) LEFT JOIN words USING (wordid) LEFT JOIN synsets USING (synsetid);
END TRANSACTION;
